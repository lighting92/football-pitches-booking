...
namespace FootballPitchesBooking.BusinessObjects
{
    public class RecommendationBO
    {
        ...
        public List<RecommendRivalModel> FindRivals(string userName)
        {
            if (!string.IsNullOrWhiteSpace(userName))
            {
                ReservationDAO resDAO = new ReservationDAO();
                var reservations = resDAO.GetAllReservationsOfUser(userName);
                var isRival = reservations.Where(r => r.NeedRival).ToList();
                var notCancelReservations = isRival.Where(r => !r.Status.ToLower().Equals("canceled")).ToList();
                var mostReservation = notCancelReservations.GroupBy(r => r.Field.StadiumId).Select(r => new { StadiumID = r.Key, Count = r.Count() }).OrderByDescending(r => r.Count).ToList();

                var reservationPriority = new List<PriorityModel>();
                return null;
